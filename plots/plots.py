...
def get_peers_plot(peers):
    ...
    for i in range(len(peers['players'])):
        plt.text(peers['games'][i] - 30, peers['players'][i], peers['games'][i], ha='center', color='black')
        plt.text(peers['won'][i] - 30, peers['players'][i], peers['won_percent'][i], ha='center', color='black')
    plt.title('Peers')
    plt.xlabel('Players')
